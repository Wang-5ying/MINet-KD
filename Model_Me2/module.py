...
import torch
from CoCA.codes.GCoNet_plus_For_Four_Model.SegFormer.mmseg.models.utils.drop import DropPath
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim=2048, embed_dim=768):
        super().__init__()
        self.proj = nn.Sequential(nn.Linear(input_dim, embed_dim),
                                  nn.GELU(),
...
class MixerBlock(nn.Module):
    ...
    def __init__(self, t_dim, c_dim, n_dim):
        super().__init__()
        self.tokens_mlp_dim = t_dim
        self.channels_mlp_dim = c_dim
        self.norm1 = nn.LayerNorm(n_dim)
        self.norm2 = nn.LayerNorm(n_dim)
        self.mlp1 = MLP(t_dim, t_dim)
        self.mlp2 = MLP(n_dim, t_dim)
...
class Attentionshallow(nn.Module):
    def __init__(self, inchannels, outchannels, stride):
        ...
        self.depth = nn.Conv2d(inchannels, inchannels, kernel_size=3, stride=stride, dilation=1, padding=1,
                               groups=inchannels)
