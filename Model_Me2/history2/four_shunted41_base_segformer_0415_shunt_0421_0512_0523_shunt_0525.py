...
from backbone.CrossViT.models.crossvit import CrossAttention
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.attn0 = CrossAttention(
            128, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0, proj_drop=0)
        self.attn1 = CrossAttention(
            256, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0, proj_drop=0)
        self.attn2 = CrossAttention(
            512, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0, proj_drop=0)
        # self.attn3 = CrossAttention(
        #     1024, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0, proj_drop=0)
        self.guide1 = torch.nn.ConvTranspose2d(512, 256, 3, 2, 1, 1, 1, True, 1)
        self.guide2 = torch.nn.ConvTranspose2d(256, 128, 3, 2, 1, 1, 1, True, 1)
        self.guide3 = torch.nn.ConvTranspose2d(128, 64, 3, 2, 1, 1, 1, True, 1)
    def forward(self, x, y):
        ...
        re = torch.cat((self.sa1(r[0]), self.sa1_2(d[0])), dim=1)
        B, C, H, W = re.size()
        re_weight = self.attn0(re.view(B, H * W, C))
        re_weight = re_weight.view(B, C, 1, 1)
        re = self.re1(re * re_weight)
        rd.append(re)

        re = torch.cat((self.sa2(r[1]), self.sa2_2(d[1])), dim=1)
        B, C, H, W = re.size()
        re_weight = self.attn1(re.view(B, H * W, C))
        re_weight = re_weight.view(B, C, 1, 1)
        re = self.re2(re * re_weight)
        rd.append(re)

        re = torch.cat((self.sa3(r[2]), self.sa3_2(d[2])), dim=1)
        B, C, H, W = re.size()
        re_weight = self.attn2(re.view(B, H * W, C))
        re_weight = re_weight.view(B, C, 1, 1)
        re = self.re3(re * re_weight)
        rd.append(re)
