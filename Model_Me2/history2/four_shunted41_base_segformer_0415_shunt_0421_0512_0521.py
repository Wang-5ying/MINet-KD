...
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.prs1 = nn.Conv2d(512, 128, 1)
        self.prs2 = nn.Conv2d(256, 64, 1)
        ...
        # self.mlp_decode1 = DecoderHead([128, 256, 512, 512])
    def forward(self, x, y):
        ...
        for blk in block:
            d1 = blk(d1, H, W)
        d1 = norm(d1)
        d1 = d1.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()  # 512, 8, 8
        decode.append(d1)

        d1 = self.prs1(F.interpolate(d1, scale_factor=4))
        ...
        d2 = self.prs2(F.interpolate(d2, scale_factor=4))
