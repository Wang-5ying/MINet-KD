...
from codes.GCoNet_plus_For_Four_Model.Model_Me.decoders.MLPDecoder import DecoderHead, DecoderHead_Mul
...
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.aspp3 = ASPP(128, [7, 5, 3, 1], 64)


        self.n1 = nn.Conv2d(256 * 3, 256, 1)
        self.n2 = nn.Conv2d(128 * 3, 128, 1)
        self.n3 = nn.Conv2d(64 * 3, 64, 1)
        # edge
        self.edge = BasicConv2d(128, 128, 3, 1, 1)
        self.edgeout = BasicConv2d(128, 1, 1)

        self.mlp_decode = DecoderHead_Mul([64, 128, 256, 512])
        self.mlp_d1 = nn.Conv2d(64, 64, 1)
        self.mlp_d2 = nn.Conv2d(128, 128, 1)
        self.mlp_d3 = nn.Conv2d(256, 256, 1)
        self.mlp_d4 = nn.Conv2d(512, 512, 1)
    def forward(self, x, y):
        ...
        y = self.aspp1(d0)
        # d1 = x * y + x + y
        d1 = self.n1(torch.cat((x * y, x, y), dim=1))
        decode.append(d1)
        ...
        y = self.aspp2(d1)
        # d2 = x * y + x + y
        d2 = self.n2(torch.cat((x * y, x, y), dim= 1))
        decode.append(d2)
        ...
        y = self.aspp3(d3)
        # d2 = x * y + x + y
        d3 = self.n3(torch.cat((x * y, x, y), dim=1))
        decode.append(d3)
        # print(d3.size(),"d3")

        decoder = [d0, d0, d0, d0]
        decoder[0] = self.mlp_d1(decode[-1])
        decoder[1] = self.mlp_d2(decode[-2])
        decoder[2] = self.mlp_d3(decode[-3])
        decoder[3] = self.mlp_d4(decode[-4])
        # for i in decoder:
        #     print(i.size())

        mlp_decode = F.interpolate(self.mlp_decode(decoder), size=256)
        # print("mlp", mlp_decode.size())
        ...
        # a = self.edge(d3)
        # edge = a - (1 - d3)
        # edge = F.interpolate(self.edgeout(edge), size=256)
        # a = self.edge(d3)
        # # edge = a - (1 - d3)
        # edge = F.interpolate(self.edgeout(a), size=256)
        # r
        returnr = []
        B, C, H, W = r[-2].size()
