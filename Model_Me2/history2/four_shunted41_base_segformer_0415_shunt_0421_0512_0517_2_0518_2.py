...
from backbone.DIFFormer.nodeclassification.difformer import DIFFormer
class PSDM(nn.Module):
    ...
    def __init__(self, input_dim1, input_dim2, input_dim3):
        super().__init__()
        self.proj1 = nn.Conv2d(input_dim1, input_dim1, 1)
        self.proj2 = nn.Conv2d(input_dim2, input_dim2, 1)
        self.proj3 = nn.Conv2d(input_dim3, input_dim3, 1)
        self.proj4 = nn.Conv2d(input_dim1 + input_dim2 + input_dim3, 1, 1)
        self.bn = nn.BatchNorm2d(1)
        # self.act = nn.Tanh()
    def forward(self, x, y, z):  # c  h  w
        x = self.proj1(x)
        y = F.interpolate(self.proj2(y), scale_factor=2)
        z = F.interpolate(self.proj3(z), scale_factor=4)
        res = F.interpolate(self.proj4(torch.cat((x, y, z), dim=1)), scale_factor=4)
        # res = self.act(res)
        return self.bn(res)
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.re4 = nn.Conv2d(512 * 2, 64, 1)
        self.diff = DIFFormer(64 * 8 * 8, 32 * 8, 64 * 8 * 8)
        ...
        self.DD1 = DD(64, 256, [4, 2], [8, 8], [3, 2], [2, 4])
        self.DD2 = DD(64, 128, [8, 4], [16, 16], [7, 3], [2, 2])
        self.DD3 = DD(64, 64, [16, 8], [32, 32], [15, 7], [2, 2])
