...
from backbone.Segnext import segnext_b
...
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.resnet = segnext_b()
        self.resnet2 = segnext_b()
    ...
    def forward(self, x, y):
        ...
        re = self.sa1(r[0]) + self.sa1_2(d[0])
        rd.append(re)
        re = self.sa2(r[1]) + self.sa2_2(d[1])
        rd.append(re)
        re = self.sa3(r[2]) + self.sa3_2(d[2])
        rd.append(re)
        re = self.sa4(r[3]) + self.sa4_2(d[3])
        rd.append(re)
        # for i in rd:
        #     print("rd", i.size())
        d0 = self.doc(r[-1] + d[-1])
