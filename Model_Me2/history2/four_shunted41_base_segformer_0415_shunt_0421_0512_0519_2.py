...
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.cr0 = MixerBlock(512, 256, 64)   # 0519_2
        self.cr1 = MixerBlock(256, 128, 256)  # 16 * 16
        self.cr2 = MixerBlock(128, 64, 1024)
        self.cr3 = MixerBlock(64, 32, 4096)
    ...
    def forward(self, x, y):
        ...
        re = self.cr3(self.re1(torch.cat((self.sa1(r[0]), self.sa1_2(d[0])), dim=1)))
        rd.append(re)
        re = self.cr2(self.re2(torch.cat((self.sa2(r[1]), self.sa2_2(d[1])), dim=1)))
        rd.append(re)
        re = self.cr1(self.re3(torch.cat((self.sa3(r[2]), self.sa3_2(d[2])), dim=1)))
        rd.append(re)
        re = self.cr0(self.re4(torch.cat((self.sa4(r[3]), self.sa4_2(d[3])), dim=1)))
        rd.append(re)
        # for i in rd:
        #     print("rd", i.size())
        decode = []
