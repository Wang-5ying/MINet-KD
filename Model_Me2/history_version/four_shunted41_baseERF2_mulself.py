...
from codes.GCoNet_plus_For_Four_Model.Model_Me.module import DD, SpatialAttention, CM, BasicConv2d, IERD, Attentionshallow, predictionshallow
...
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.IED1 = IERD(128, 128, [3, 5, 7], 1, [1, 2], [1, 1])
        self.IER1 = IERD(128, 128, [3, 5, 7], 1, [1, 2], [1, 1])
        self.IED2 = IERD(512, 512, [3, 5, 7], 1, [1, 2], [1, 1])
        self.IER2 = IERD(512, 512, [3, 5, 7], 1, [1, 2], [1, 1])
        ...
        self.aft0 = BasicConv2d(512, 512, 3, 1, 1)
        self.aft1 = BasicConv2d(256, 256, 3, 1, 1)
        self.aft2 = BasicConv2d(128, 128, 3, 1, 1)
        ...
        self.atten1 = Attentionshallow(128, 128, 1)
        self.atten2 = Attentionshallow(64, 128, 1)
        self.atten3 = Attentionshallow(128, 256, 1)


        self.supshallow1 = predictionshallow(128, 64, 1)
        self.supshallow2 = predictionshallow(64, 128, 2)
        self.supshallow3 = predictionshallow(128, 256, 2)
    ...
    def forward(self, x, y):
        ...
        # ------ colomn3------
        x = self.IER1(x_he) * x + x
        y = self.IED1(y_he) * y + x
        # r_stage3
        B, C, H, W = x.size()
        ...
        x = x * self.IER2(x_he) + x
        r.append(x)
        y = y * self.IED2(y_he) + x
        d.append(y)
        ...
        d1 = self.aft0(torch.cat((self.DD1(rd[-1], rd[-2]), d0), dim=1))

        d1 = self.prs1(F.interpolate(d1, scale_factor=2))
        d2 = self.aft1(torch.cat((self.DD2(rd[-1], rd[-3]), d1), dim=1))

        d2 = self.prs2(F.interpolate(d2, scale_factor=2))
        d3 = self.aft2(torch.cat((self.DD3(rd[-1], rd[-4]), d2), dim=1))
        # print(d3.size(),"d3")
        ...
        # print(d0.size(), d1.size(), d2.size(), d3.size())
        # mul_self_disitllation_attention  ----- images flow
        re = []
        x = self.supshallow1(self.atten1(d3))
        re.append(x)

        x = self.supshallow2(self.atten2(d2))
        re.append(x)

        x = self.supshallow3(self.atten1(d1))
        re.append(x)
        ...
        # for i in re:
        #     print("shallow", i.size())
        # for i in rei:
        #     print("rei", i.size())
        if self.training:
            return returnvalues, returnr, returnd, re, rei
        else:
            return returnvalues
