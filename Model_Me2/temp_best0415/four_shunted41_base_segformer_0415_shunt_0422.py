...
class Me(nn.Module):
    ...
    def forward(self, x, y):
        ...
        a = self.sa1(r[0])
        b = self.sa1_2(d[0])
        re = self.sig(self.re1(torch.cat((a, b), dim=1)))
        re = re * a + re * b
        rd.append(re)
        a = self.sa2(r[1])
        b = self.sa2_2(d[1])
        re = self.sig(self.re2(torch.cat((a, b), dim=1)))
        re = re * a + re * b
        rd.append(re)
        a = self.sa3(r[2])
        b = self.sa3_2(d[2])
        re = self.sig(self.re3(torch.cat((a, b), dim=1)))
        re = re * a + re * b
        rd.append(re)
        a = self.sa4(r[3])
        b = self.sa4_2(d[3])
        re = self.sig(self.re4(torch.cat((a, b), dim=1)))
        re = re * a + re * b
        rd.append(re)
        # for i in rd:
        #     print("rd", i.size())
