...
class Me(nn.Module):
    ...
    def forward(self, x1, y, gt):
        ...
        d.append(y)
        if self.training:
            r_gt = self.resnet.forward_features(x1*gt)
            r_back = self.resnet.forward_features(x1*(1-gt))
        # for i in r:
        #     print(i.size())
        ...
        if self.training:
            return returnvalues, r, d, cls, decode, w, relation, r_gt, r_back# , rd
        else:
            return returnvalues
