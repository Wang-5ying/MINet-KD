...
class Me(nn.Module):
    def load_pre(self, pre_model1):
        ...
        # self.resnet2.load_state_dict(new_state_dict3, strict=False)
        self.resnet3.load_state_dict(new_state_dict3, strict=False)
        self.encoder.load_pre(pre_model1)
        print(f"RGB SwinTransformer loading pre_model ${pre_model1}")
        print(f"Depth SwinTransformer loading pre_model ${pre_model1}")
    ...
    def forward(self, x, y, gt):
        B = x.shape[0]
        r, d, w = self.encoder(x, y, gt)
        if self.training:
            ryes, dyes, wyes = self.encoder(x*gt, y*gt, gt)
            # rno, dno, wno = self.encoder(x*(1-gt), y*(1-gt), gt)
        rd = []
        ...
        if self.training:
            return returnvalues, r, d, cls, decode, w, relation, clsyes, clsyno, ryes, dyes, wyes#, rno, dno, wno# , rd
        else:
            return returnvalues
