...
from CoCA.codes.GCoNet_plus_For_Four_Model.Model_Me.decoders.MLPDecoder import DecoderHead
#############630 qudiaol dongtaijuanji   vt5000 0.026 150epoch
from CoCA.codes.GCoNet_plus_For_Four_Model.Model_Me.module import DD, SpatialAttention, CM, BasicConv2d, SELayer, ASPP, MixerBlock, SpatialAttention, CA, MAttention
# from backbone.Shunted_Transformer.SSA import shunted_b
# from mmseg.models.backbones.mix_transformer import mit_b4
from CoCA.backbone.Shunted_Transformer.SSA import shunted_t
from CoCA.文献代码.FcaNet.model.layer import MultiSpectralAttentionLayer
from CoCA.codes.GCoNet.models.GCoNet import AllAttLayer
...
class Me(nn.Module):
    ...
    def __init__(self, mode='small'):
        ...
        self.clsy = nn.Conv2d(256, 1, 1)
        self.adp1avgy = nn.AdaptiveAvgPool2d(32)
        self.clsn = nn.Conv2d(256, 1, 1)
        self.adp1avgn = nn.AdaptiveAvgPool2d(32)
        self.sig = nn.Sigmoid()
    def forward(self, x, y, gt):
        ...
        global1_gt = self.glo(rd[-1] * F.interpolate(gt, size=8))
        global1_back = self.glo(rd[-1] * F.interpolate(1-gt, size=8))
        scl = []
        scl.append(global1)
        scl.append(global1_gt)
        scl.append(global1_back)
        ...
        if self.training:
            clsyes = self.sig(self.adp1avgy(self.clsy(global1 * F.interpolate(gt, size=16))))
            clsyno = self.sig(self.adp1avgn(self.clsn(global1 * (1-F.interpolate(gt, size=16)))))
        ...
        # if self.training:
        #     return returnvalues, r, d, cls, decode, w, relation, clsyes, clsyno# , rd
        # else:
        #     return returnvalues
        # return returnvalues, r, d, cls
        if self.training:
            return returnvalues, r, d, cls, decode, w, relation, clsyes, clsyno, globali, scl# , rd
        # else:
        else:
            return returnvalues
...
if __name__ == '__main__':
    ...
    c = torch.randn(2, 1, 256, 256).cuda()
    model = Me()
    model.cuda()
    # model.load_pre("/media/wby/shuju/ckpt_B.pth")
    out = model(a, b, c)
    flops, params = profile(model, (a, a, c))
