...
def main(args):
    ...
    for testset in args.testsets.split('+'):
        ...
        for batch in tqdm(test_loader):
            ...
            ori_sizes = batch[-1]
            with torch.no_grad():
                scaled_preds = model(inputs, depths)[4]  # 取得模型的最后一个输出作为预测结果
            ...
            for inum in range(num):
                ...
                if config.db_output_refiner or (not config.refine and config.db_output_decoder):
                    ...
                    res = cv2.applyColorMap(out_img, cv2.COLORMAP_JET)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--testsets',
                        default='RGBD_CoSal1k+RGBD_CoSal150+RGBD_CoSeg183',
    ...
    parser.add_argument('--pred_dir',
                        default='/media/wby/shuju/ckpt/M/RGBD_CoSal150/w/r+d_new/4/',
