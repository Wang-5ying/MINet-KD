...
class FcaBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, h, w, stride=1, downsample=None, groups=1,
        ...
        self.conv3 = nn.Conv2d(inplanes, inplanes * 4, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(inplanes * 4)
        self.relu = nn.ReLU(inplace=True)
        self.att = MultiSpectralAttentionLayer(inplanes * 4, h, h,  reduction=reduction, freq_sel_method = 'top16')

        self.downsample = downsample
        self.stride = stride
        self.conv4 = nn.Conv2d(inplanes * 4, inplanes, kernel_size=1, bias=False)
    def forward(self, x):
        residual = x
