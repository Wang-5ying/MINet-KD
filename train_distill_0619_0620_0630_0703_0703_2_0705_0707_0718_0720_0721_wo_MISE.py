...
from codes.GCoNet_plus_For_Four_Model.Model_Me.best_0601.student.four_shunted41_base_segformer_0415_shunt_0421_0512_0523_shunt_0526_0527_2_0528_0601_0703_0721_ablation_backbone import \
    Me as S
...
for testset in args.testsets.split('+'):
    test_loader = get_loader(
        os.path.join('/data', 'images', testset),
        os.path.join('/data', 'gts', testset),
        os.path.join('/data', 'gts', testset),
        os.path.join('/data', 'gts', testset),
        os.path.join('/data', 'gts', testset),
        os.path.join('/data', 'depths', testset),
        os.path.join('/data', 'gts', testset),
...
def train(epoch, temperature):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # print("gt", gts)
        with torch.no_grad():
            Treturnvalues, Treturnr, Treturnd, Tcls, Tdecode, Tgatc, Tw, Trelation, Tglobal = teachers(inputs, depths)
        # for i in teacher_rte:
        #     print("teacher", i.size())

        returnvalues, returnr, returnd, cls, decode, w, rel, clsy, clsn, globali, scl = model2(inputs, depths, gts)

        scl = sclloss(scl[0], scl[1], scl[2])

        proto1 = sclloss_op(globali[0], Tglobal[0])
        proto2 = sclloss_op(globali[1], Tglobal[1])
        proto3 = sclloss_op(globali[2], Tglobal[2])
        # proto4 = sclloss_op(globali[3], Tglobal[3])

        prot = proto3 + proto2 + proto1
        ...
        kl13 = sclloss_op(returnr[0] / temperature, Treturnr[0] / temperature)
        kl14 = sclloss_op(returnr[1] / temperature, Treturnr[1] / temperature)
        kl15 = sclloss_op(returnr[2] / temperature, Treturnr[2] / temperature)
        kl16 = sclloss_op(returnr[3] / temperature, Treturnr[3] / temperature)

        kl17 = sclloss_op(returnd[0] / temperature, Treturnr[0] / temperature)
        kl18 = sclloss_op(returnd[1] / temperature, Treturnr[1] / temperature)
        kl19 = sclloss_op(returnd[2] / temperature, Treturnr[2] / temperature)
        kl20 = sclloss_op(returnd[3] / temperature, Treturnr[3] / temperature)

        kl21 = sclloss_op(w[0] / temperature, Treturnr[0] / temperature)
        kl22 = sclloss_op(w[1] / temperature, Treturnr[1] / temperature)
        kl23 = sclloss_op(w[2] / temperature, Treturnr[2] / temperature)
        kl24 = sclloss_op(w[3] / temperature, Treturnr[3] / temperature)

        # kl25 = sclloss(F.interpolate(returnvalues[0], size=64) / temperature, F.interpolate(Treturnvalues[0], size=64) / temperature, F.interpolate((1 - gts), size=64) / temperature)
        # kl26 = sclloss(F.interpolate(returnvalues[1], size=64) / temperature, F.interpolate(Treturnvalues[1], size=64) / temperature, F.interpolate((1 - gts), size=64) / temperature)
        # kl27 = sclloss(F.interpolate(returnvalues[2], size=64) / temperature, F.interpolate(Treturnvalues[2], size=64) / temperature, F.interpolate((1 - gts), size=64) / temperature)
        # kl28 = sclloss(F.interpolate(returnvalues[3], size=64) / temperature, F.interpolate(Treturnvalues[3], size=64) / temperature, F.interpolate((1 - gts), size=64) / temperature)

        # new =   (kl28) / 10e8 # kl25 + kl26 + kl27 +
        feature_loss =  kl13 + kl14 + kl15 + kl16 + kl17 + kl18 + kl19 + kl20 + kl21 + kl22 + kl23 + kl24
        ...
        loss1_te = kld(returnvalues[0] / 4, Treturnvalues[0] / 4, gts, 3)  # 软目标

        loss2_gt = CE(returnvalues[1], gts)  # 硬目标
        loss2_te = kld(returnvalues[1] / 4, Treturnvalues[1] / 4, gts, 3)  # 软目标

        loss3_gt = CE(returnvalues[2], gts)  # 硬目标
        loss3_te = kld(returnvalues[2] / 4, Treturnvalues[2] / 4, gts, 3)  # 软目标

        loss4_gt = CE(returnvalues[3], gts)  # 硬目标
        loss4_te = kld(returnvalues[3] / 4, Treturnvalues[3] / 4, gts, 3)  # 软目标

        loss5_gt = CE(cls, cls_percentage)  # 硬目标
        loss5_te = kld(cls / 4, Tcls / 4, cls_percentage, 3)  # 软目标
        #
        flag1 = False
        flag2 = False
        flag3 = False
        ...
        # print(fgd1, "fgd1")
        # if epoch < 100:
        # print(feature_loss*10e-8, tradition_loss, attloss * 0.1, prot * 10e-8, scl * 10e-4)
        loss_sal = feature_loss * 10e-8 + tradition_loss * 2 + attloss * 0.1 + prot* 10e-8 + scl * 10e-4# + new
        # else:
