...
from codes.GCoNet_plus_For_Four_Model.Model_Me.best_0601.student.four_shunted41_base_segformer_0415_shunt_0421_0512_0523_shunt_0526_0527_2_0528_0601_0703 import \
    Me as S
...
# model2.load_pre("/media/wby/shuju/four/temp/best_ep77_Smeasure0.8571.pth")
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, emb_i, emb_j):  # emb_i, emb_j 是来自同一图像的两种不同的预处理方法得到
        emb_i = emb_i.view(emb_i.size()[0], -1)
        emb_j = emb_j.view(emb_j.size()[0], -1)
        ...
        # print(self.negatives_mask.size(), torch.exp(similarity_matrix / self.temperature).size())
...
SiTloss = ContrastiveLoss(batch_size=4)
...
def train(epoch, temperature):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        returnvalues, returnr, returnd, cls, decode, w, rel, clsy, clsn = model2(inputs, depths, gts)
        # returnvalues2, returnr2, returnd2, cls2, decode2, w2, rel2, clsy2, clsn2 = model2(inputs*(1-gts), depths*(1-gts), gts)
        # selfcl = sclloss(cls, clsy, clsn)/10000000
        # print(selfcl)
        # logit distillation
        # kl1 = sclloss_op(returnvalues[0] / temperature, Treturnvalues[0] / temperature)
        # kl2 = sclloss_op(returnvalues[1] / temperature, Treturnvalues[1] / temperature)
        # kl3 = sclloss_op(returnvalues[2] / temperature, Treturnvalues[2] / temperature)
        # kl4 = sclloss_op(returnvalues[3] / temperature, Treturnvalues[3] / temperature)
        # # kla = kld(returnvalues[4] / temperature, Treturnvalues[4] / temperature, gts / temperature, 3)
        # logit_loss = kl1 + kl2 + kl3 + kl4 # + kla

        # feature distillation
        # part 1 decoder
        kl5 = SiTloss(decode[0] / temperature, Tdecode[0] / temperature)+sclloss_op(decode[0] / temperature, Tdecode[0] / temperature)
        kl6 = SiTloss(decode[1] / temperature, Tdecode[1] / temperature)+sclloss_op(decode[1] / temperature, Tdecode[1] / temperature)
        kl7 = SiTloss(decode[2] / temperature, Tdecode[2] / temperature)+sclloss_op(decode[2] / temperature, Tdecode[2] / temperature)
        kl8 = SiTloss(decode[3] / temperature, Tdecode[3] / temperature)+sclloss_op(decode[3] / temperature, Tdecode[3] / temperature)
        # klb = sclloss_op(decode[4] / temperature, Tdecode[4] / temperature)

        kl13 = SiTloss(returnr[0] / temperature, Treturnr[0] / temperature)+  sclloss_op(returnr[0] / temperature, Treturnr[0] / temperature)
        kl14 = SiTloss(returnr[1] / temperature, Treturnr[1] / temperature)+  sclloss_op(returnr[1] / temperature, Treturnr[1] / temperature)
        kl15 = SiTloss(returnr[2] / temperature, Treturnr[2] / temperature)+  sclloss_op(returnr[2] / temperature, Treturnr[2] / temperature)
        kl16 = SiTloss(returnr[3] / temperature, Treturnr[3] / temperature)+  sclloss_op(returnr[3] / temperature, Treturnr[3] / temperature)

        kl17 = SiTloss(returnd[0] / temperature, Treturnr[0] / temperature)+sclloss_op(returnd[0] / temperature, Treturnr[0] / temperature)
        kl18 = SiTloss(returnd[1] / temperature, Treturnr[1] / temperature)+sclloss_op(returnd[1] / temperature, Treturnr[1] / temperature)
        kl19 = SiTloss(returnd[2] / temperature, Treturnr[2] / temperature)+sclloss_op(returnd[2] / temperature, Treturnr[2] / temperature)
        kl20 = SiTloss(returnd[3] / temperature, Treturnr[3] / temperature)+sclloss_op(returnd[3] / temperature, Treturnr[3] / temperature)

        kl21 = SiTloss(w[0] / temperature, Treturnr[0] / temperature)+sclloss_op(w[0] / temperature, Treturnr[0] / temperature)
        kl22 = SiTloss(w[1] / temperature, Treturnr[1] / temperature)+sclloss_op(w[1] / temperature, Treturnr[1] / temperature)
        kl23 = SiTloss(w[2] / temperature, Treturnr[2] / temperature)+sclloss_op(w[2] / temperature, Treturnr[2] / temperature)
        kl24 = SiTloss(w[3] / temperature, Treturnr[3] / temperature)+sclloss_op(w[3] / temperature, Treturnr[3] / temperature)

        feature_loss = kl5 + kl6 + kl7 + kl8 + kl13 + kl14 + kl15 + kl16 + kl17 + kl18 + kl19 + kl20 + kl21 + kl22 + kl23 + kl24
        ...
        loss1_te = SiTloss(returnvalues[0] / 4, Treturnvalues[0] / 4) + kld(returnvalues[0] / 4, Treturnvalues[0] / 4, gts, 3)# 软目标

        loss2_gt = CE(returnvalues[1], gts)  # 硬目标
        loss2_te = SiTloss(returnvalues[1] / 4, Treturnvalues[1] / 4) + kld(returnvalues[1] / 4, Treturnvalues[1] / 4, gts, 3)# 软目标

        loss3_gt = CE(returnvalues[2], gts)  # 硬目标
        loss3_te = SiTloss(returnvalues[2] / 4, Treturnvalues[2] / 4)  +kld(returnvalues[2] / 4, Treturnvalues[2] / 4, gts, 3)# 软目标

        loss4_gt = CE(returnvalues[3], gts)  # 硬目标
        loss4_te = SiTloss(returnvalues[3] / 4, Treturnvalues[3] / 4) + kld(returnvalues[3] / 4, Treturnvalues[3] / 4, gts, 3)# 软目标

        loss5_gt = CE(cls, cls_percentage)  # 硬目标
        loss5_te = SiTloss(cls / 4, Tcls / 4)  + kld(cls / 4, Tcls / 4, cls_percentage, 3)# 软目标
        # print(loss5_te, loss4_te, loss3_te, loss2_te, loss1_te)
        #
        flag1 = False
        flag2 = False
        flag3 = False
        ...
        loss_sal1 = loss1_gt * 0.0625 + loss1_te * (1 - 0.0625)

        loss_sal2 = loss2_gt * 0.0625 + loss2_te * (1 - 0.0625)

        loss_sal3 = loss3_gt * 0.0625 + loss3_te * (1 - 0.0625)

        loss_sal4 = loss4_gt * 0.0625 + loss4_te * (1 - 0.0625)

        cls_loss = + loss5_gt * 0.0625 + loss5_te * (1 - 0.0625)
        ...
        # print(fgd1, "fgd1")
        # if epoch < 100:
        # print(feature_loss*10e-8, tradition_loss*10e-6, attloss * 0.1 )
        loss_sal = feature_loss * 10e-8 + tradition_loss * 10e-6 + attloss * 0.1
        # else:
