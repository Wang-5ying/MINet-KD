...
def sclloss_op(x, xt):
    ...
    return loss.sum()
def sclloss_neg(x, xb):
    cosb = (1 + compute_cos_dis(x, xb)) * 0.5
    loss = - torch.log(1 - cosb + 1e-5)
    return loss.sum()
...
def train(epoch, temperature):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        returnvalues, returnr, returnd, cls, decode, w = model2(inputs, depths)

        kl5 = kld(decode[0] / temperature, Tdecode[0] / temperature, gts, 3)
        kl6 = kld(decode[1] / temperature, Tdecode[1] / temperature, gts, 3)
        kl7 = kld(decode[2] / temperature, Tdecode[2] / temperature, gts, 3)
        kl8 = kld(decode[3] / temperature, Tdecode[3] / temperature, gts, 3)
        # klb = sclloss_op(decode[4] / temperature, Tdecode[4] / temperature)
        # op1 = sclloss_op(w[-1]/temperature, returnr[-1] / temperature)
        # op2 = sclloss_op(w[-1]/temperature, returnd[-1] / temperature)
        # print(w[-1][0].size(), w[-1].size())
        op3 = sclloss_op(w[-1][0]/temperature, Tw[-1][0] / temperature) + sclloss_op(w[-1][1]/temperature, Tw[-1][1] / temperature)  \
              + sclloss_op(w[-1][2]/temperature, Tw[-1][2] / temperature) + sclloss_op(w[-1][3]/temperature, Tw[-1][3] / temperature)
        op4 = sclloss_op(returnr[-1][0]/temperature, Treturnr[-1][0] / temperature) + sclloss_op(returnr[-1][1]/temperature, Treturnr[-1][1] / temperature) + sclloss_op(returnr[-1][2]/temperature, Treturnr[-1][2] / temperature) \
                + sclloss_op(returnr[-1][3]/temperature, Treturnr[-1][3] / temperature)
        op5 = sclloss_op(returnd[-1][0]/temperature, Treturnd[-1][0] / temperature) + sclloss_op(returnd[-1][1]/temperature, Treturnd[-1][1] / temperature) + sclloss_op(returnd[-1][2]/temperature, Treturnd[-1][2] / temperature) \
                + sclloss_op(returnd[-1][3]/temperature, Treturnd[-1][3] / temperature)
        # op6 = sclloss_op(returnr[-1]/temperature, returnd[-1] / temperature)
        opstive = op3 + op4 + op5

        # negtive1 = 0
        # for i in w[-1]:
        #     for j in w[-1]:
        #         negtive1 = negtive1 + sclloss_neg(i, j)
        # negtive1 = negtive1 - sclloss_neg(w[-1][0], w[-1][0]) - sclloss_neg(w[-1][1], w[-1][1])- sclloss_neg(w[-1][2], w[-1][2])- sclloss_neg(w[-1][3], w[-1][3])
        #
        # negtive2 = 0
        # for i in returnr[-1]:
        #     for j in returnr[-1]:
        #         negtive2 = negtive2 + sclloss_neg(i, j)
        # negtive2 = negtive2 - sclloss_neg(returnr[-1][0], returnr[-1][0]) - sclloss_neg(returnr[-1][1], returnr[-1][1]) -  sclloss_neg(returnr[-1][2], returnr[-1][2]) - sclloss_neg(returnr[-1][3], returnr[-1][3])
        ...
        tradition_loss = loss_sal4 + cls_loss

        # print(negtive1 / 100000 , negtive2 / 100000)
        loss_sal = tradition_loss + feature_loss*10e-9 + opstive / 10000
        loss = loss + loss_sal
        # teacher student
