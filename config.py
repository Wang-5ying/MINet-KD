...
class Config():
    def __init__(self) -> None:
        ...
        # Augmentation
        self.preproc_methods = ['flip', 'enhance', 'rotate', 'crop', 'pepper'][:3]  # 原
        # self.preproc_methods = ['flip', 'enhance', 'rotate', 'pepper', 'crop', 'random_gaussian'][:4]  # 0325 加入噪点 -- 会变差
        ...
        # Refiner
        self.refine = [0, 1, 4][
            1]  # 0 -- no ref`  inement, CoCA -- only output mask for refinement, 4 -- but also raw input.   ###not found any explanation whether there is a refine
        if self.refine:
            self.batch_size = 4 # self.batch_size = 16  8 # teacher 5   STUDENT 24  best 5  # 因为confer跑步下，由5改为4 ！！！！！！！！！！！  four_teacher:4
        else:
            if self.bb != 'vgg16':
        ...
        # 3("self.db_output_refiner",self.db_output_refiner)
        ...
        self.lr =  3e-5  # teacher 3e - 5  # 3e-5
        ...
        self.rand_seed = 7  # 0411

        # print(self.rand_seed,"rand_sedd")
