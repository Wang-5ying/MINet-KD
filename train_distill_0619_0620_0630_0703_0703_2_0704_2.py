...
from codes.GCoNet_plus_For_Four_Model.Model_Me.best_0601.student.four_shunted41_base_segformer_0415_shunt_0421_0512_0523_shunt_0526_0527_2_0528_0601_0703_0704 import \
    Me as S
...
def train(epoch, temperature):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        returnvalues, returnr, returnd, cls, decode, w, rel, clsy, clsn, ryes, dyes, wyes = model2(inputs, depths, gts)
        selfcl1 = sclloss(cls, clsy, clsn)/10000000
        featureloss = sclloss_part(returnr[-1], ryes[-1])/1000
        selfcl = selfcl1 + featureloss
        number = 0
        # while number < 4:
        # featurecl_r = sclloss(returnr[-2], ryes[-2], rno[-2])
            # featurecl_d = sclloss(returnd[number], dyes[number], dno[number])
            # featurecl_w = sclloss(w[number], wyes[number], wno[number])
        # featureloss = featureloss + featurecl_r #+ featurecl_d + featurecl_w
        # number = number + 1
        # print(featureloss, selfcl1)
        # print(selfcl)
        # logit distillation
        ...
        kl17 = kld(returnd[0] / temperature, Treturnd[0] / temperature, gts / temperature, 3)
        kl18 = kld(returnd[1] / temperature, Treturnd[1] / temperature, gts / temperature, 3)
        kl19 = kld(returnd[2] / temperature, Treturnd[2] / temperature, gts / temperature, 3)
        kl20 = kld(returnd[3] / temperature, Treturnd[3] / temperature, gts / temperature, 3)

        kl21 = kld(w[0] / temperature, Tw[0] / temperature, gts / temperature, 3)
        kl22 = kld(w[1] / temperature, Tw[1] / temperature, gts / temperature, 3)
        kl23 = kld(w[2] / temperature, Tw[2] / temperature, gts / temperature, 3)
        kl24 = kld(w[3] / temperature, Tw[3] / temperature, gts / temperature, 3)
        ...
        loss1_te = kld(returnvalues[0], Treturnvalues[0]/4, gts, 3)  # 软目标

        loss2_gt = CE(returnvalues[1], gts)  # 硬目标
        loss2_te = kld(returnvalues[1], Treturnvalues[1]/4, gts, 3)  # 软目标

        loss3_gt = CE(returnvalues[2], gts)  # 硬目标
        loss3_te = kld(returnvalues[2], Treturnvalues[2]/4, gts, 3)  # 软目标

        loss4_gt = CE(returnvalues[3], gts)  # 硬目标
        loss4_te = kld(returnvalues[3], Treturnvalues[3]/4 ,gts, 3)  # 软目标
        ...
        cls_loss = + loss5_te * (1 - 0.0625) #+ loss5_te * (1 - 0.0625)
        ...
        # print(fgd1, "fgd1")
        # if epoch < 100:
        loss_sal = logit_loss + feature_loss + tradition_loss + attloss * 0.1 + selfcl
        # else:
