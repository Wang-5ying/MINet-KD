...
class CoData(data.Dataset):
    def __init__(self, image_root, label_root, softgt_root, softgt2_root, softgt3_root, depth_root, edge_root, image_size, max_num, is_train):
        ...
        self.softgt2_dirs = list(map(lambda x: os.path.join(softgt2_root, x), class_list))
        self.softgt3_dirs = list(map(lambda x: os.path.join(softgt3_root, x), class_list))
        ...
        self.transform_softgt2 = transforms.Compose([
            transforms.PILToTensor(),
            transforms.Resize(self.data_size),
        ])
        self.transform_softgt3 = transforms.Compose([
            transforms.Resize(self.data_size),
            transforms.ToTensor(),
        ])
    ...
    def __getitem__(self, item):
        ...
        softgt2_paths = list(map(lambda x: os.path.join(self.softgt2_dirs[item], x[:-4] + '.png'), names))
        softgt3_paths = list(map(lambda x: os.path.join(self.softgt3_dirs[item], x[:-4] + '.png'), names))
        ...
        if self.is_train:
            ...
            new_softgt2_paths = [softgt2_paths[i] for i in sampled_list]
            new_softgt3_paths = [softgt3_paths[i] for i in sampled_list]
            ...
            softgt_paths = new_softgt_paths
            softgt2_paths = new_softgt2_paths
            softgt3_paths = new_softgt3_paths
        ...
        softgts = torch.Tensor(final_num, 1, self.data_size[1], self.data_size[0])
        softgt2s = torch.Tensor(final_num, 1, self.data_size[1], self.data_size[0])
        softgt3s = torch.Tensor(final_num, 1, self.data_size[1], self.data_size[0])
        ...
        for idx in range(final_num):
            if self.load_all:
                ...
                softgt2 = self.softgt2_loaded[idx]
                softgt3 = self.softgt3_loaded[idx]
                ...
                if not os.path.exists(softgt2_paths[idx]):
                    softgt2_paths[idx] = softgt2_paths[idx].replace('.jpg', '.png') if softgt2_paths[idx][-4:] == '.jpg' else \
                    softgt2_paths[idx].replace('.png', '.jpg')
                softgt2 = Image.open(softgt2_paths[idx]).convert('L')
                if not os.path.exists(softgt3_paths[idx]):
                    softgt3_paths[idx] = softgt3_paths[idx].replace('.jpg', '.png') if softgt3_paths[idx][-4:] == '.jpg' else \
                    softgt3_paths[idx].replace('.png', '.jpg')
                softgt3 = Image.open(softgt3_paths[idx]).convert('L')
            ...
            image, label, softgt, softgt2, softgt3,depth, edge = self.transform_image(image), self.transform_label(label), self.transform_softgt(softgt), self.transform_softgt2(softgt2), self.transform_softgt2(softgt2), self.transform_depth(depth), self.transform_edge(edge)
            images[idx] = image
            labels[idx] = label
            softgts[idx] = softgt
            softgt2s[idx] = softgt2
            softgt3s[idx] = softgt3
            depths[idx] = depth
            edges[idx] = edge

        if self.is_train:
            cls_ls = [item] * final_num
            return images, labels, softgts, softgt2s, softgt3s, depths, edges, subpaths, ori_sizes, cls_ls
        else:
            return images, labels, softgts, softgt2s, softgt3s, depths, edges, subpaths, ori_sizes

    def __len__(self):
        return len(self.image_dirs)


def get_loader(img_root, gt_root, softgt_root, softgt2_root, softgt3_root, depth_root, edge_root, img_size, batch_size, max_num=float('inf'), istrain=True, shuffle=False,
               num_workers=0, pin=False):
    # print(depth_root,"jhhhhh")
    dataset = CoData(img_root, gt_root, softgt_root, softgt2_root, softgt3_root, depth_root, edge_root, img_size, max_num, is_train=istrain)
